...
function reverseInteger(num) {
  const oldStr = num.toString();
  let newStr = '';
  for (let i = 0; i < oldStr.length; i += 1) {
    newStr += oldStr[oldStr.length - i - 1];
  }
...
function getCommonDirectoryPath(pathes) {
  const pathArr = pathes.map((element) => element.split('/'));
  let output = '';
  for (let i = 0; i < pathArr[0].length; i += 1) {
    if (pathArr.every((item) => pathArr[0][i] === item[i])) {
      output += pathArr[0][i];
      output += '/';
    } else {
      break;
